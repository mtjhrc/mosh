...
private:
  ...
  public:
    ...
    Packet( Direction s_direction,
            uint64_t s_seq,
            uint16_t s_timestamp,
            uint16_t s_timestamp_reply,
            const std::string& s_payload )
      : seq( s_seq ), direction( s_direction ), timestamp( s_timestamp ), timestamp_reply( s_timestamp_reply ),
        payload( s_payload )
    {}
  ...
  static constexpr int RECEIVE_MTU = 2048;
...
public:
  ...
  std::optional<Port> tcp_port( void ) const override { return std::nullopt; }
