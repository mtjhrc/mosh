...
std::unique_ptr<Connection> make_server_connection( Base64Key key,
...
std::unique_ptr<Connection> make_client_connection( Base64Key key,
                                                    const char* addr,
                                                    ...
                                                    NetworkTransportMode mode )
{
  switch ( mode ) {
    ...
    case NetworkTransportMode::PREFER_UDP:
      return std::make_unique<CombinedConnection>( key, addr, udp_port.value(), tcp_port.value() );
    default:
