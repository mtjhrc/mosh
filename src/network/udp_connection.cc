...
/* Read in packet */
UDPConnection::Packet::Packet( const Message& message )
  : seq( seq_from_nonce( message.nonce ) ), direction( direction_from_nonce( message.nonce ) ), timestamp( -1 ),
...
/* Output from packet */
Message UDPConnection::Packet::toMessage( void )
{
  uint16_t ts_net[2]
  ...
  return Message( make_nonce( direction, seq ), timestamps + payload );
}

UDPConnection::Packet UDPConnection::new_packet( const std::string& s_payload )
{
  ...
  UDPConnection::Packet p( direction, seq_counter.next(), timestamp16(), outgoing_timestamp_reply, s_payload );
...
{
  Addr local_addr( addr, 0, SOCK_DGRAM, true );
  ...
  for ( uint16_t i = search_low; i <= search_high; i++ ) {
    local_addr.set_port( i );
    ...
    if ( ::bind( sock(), &local_addr.sa(), local_addr.len() ) == 0 ) {
...
UDPConnection::UDPConnection( Base64Key key, const char* ip, Port port ) /* client */
  : socks(), has_remote_addr_( false ), remote_addr( ip, port.value(), SOCK_DGRAM, false ), server( false ),
    MTU( DEFAULT_SEND_MTU ), session( key ), direction( TO_SERVER ), saved_timestamp( -1 ),
...
void UDPConnection::send_fragment( const std::string& s )
{
  ...
  ssize_t bytes_sent = sendto( sock(), p.data(), p.size(), MSG_DONTWAIT, &remote_addr.sa(), remote_addr.len() );
...
std::optional<Instruction> UDPConnection::recv( void )
{
  ...
  if ( !fragments.add_fragment( frag ) ) {
    return std::nullopt;
  }
...
std::string UDPConnection::recv_one( int sock_to_recv )
{
  ...
  char msg_payload[UDPConnection::RECEIVE_MTU];
  char msg_control[UDPConnection::RECEIVE_MTU];

  /* receive source address */
  header.msg_name = &packet_remote_addr.sa();
  header.msg_namelen = packet_remote_addr.max_len();
  ...
  if ( server && /* only client can roam */
       ( remote_addr.len() != header.msg_namelen
         || memcmp( &remote_addr.sa(), &packet_remote_addr.sa(), remote_addr.len() ) != 0 ) ) {
    remote_addr = packet_remote_addr;
    remote_addr.set_len( header.msg_namelen );
    char host[NI_MAXHOST], serv[NI_MAXSERV];
...
void UDPConnection::register_select( Select& select )
{
  for ( const auto& sock : socks ) {
    select.add_read_fd( sock.fd() );
  }
}

Actions UDPConnection::wakeup( Select& select )
{
  Actions actions;
  actions.recv = true;
  for ( const auto& sock : socks ) {
    if ( select.read( sock.fd() ) ) {
      actions.recv = true;
      break;
    }
  }
...
uint64_t UDPConnection::get_RTO( void ) const
{
